...
from cords.utils.losses import *
import matplotlib.pyplot as plt
from cords.utils.data.datasets.SSL.utils import InfiniteSampler
from cords.utils.data.data_utils import WeightedDataset
...
class TrainClassifier:
    def __init__(self, config_data):
        version = config_data.loss.type + "_" + config_data.dss_args.type
    ...
    def create_model(self):
        ...
        elif self.cfg.model.architecture == 'SiameseResNet50':
            if self.cfg.model.pretrained:
                model = SiameseResNetPretrained('ResNet50', class_num=self.cfg.model.numclasses)
            else:
                model = SiameseResNet50(self.cfg.model.numclasses)
    ...
    def loss_function(self):
        if self.cfg.loss.type == "ccsa":
            pairedcriterion = CCSALoss()
            pairedcriterion_nored = CCSALoss(reduction='none')
            criterion = nn.CrossEntropyLoss()
            criterion_nored = nn.CrossEntropyLoss(reduction='none')
        elif self.cfg.loss.type == "dsne":
            pairedcriterion = DSNELoss()
            pairedcriterion_nored = DSNELoss(reduction='none')
            criterion = nn.CrossEntropyLoss()
            criterion_nored = nn.CrossEntropyLoss(reduction='none')
        return pairedcriterion, pairedcriterion_nored, criterion, criterion_nored
    ...
    def train(self):
        ...
        # Loss Functions
        pairedcriterion, pairedcriterion_nored, criterion, criterion_nored = self.loss_function()
        ...
        if self.cfg.dss_args.type in ['GradMatch', 'GradMatchPB', 'GradMatch-Warm', 'GradMatchPB-Warm']:
            ...
            """
            ############################## SMI Dataloader Additional Arguments ##############################
            """
            self.cfg.dss_args.model = model
            self.cfg.dss_args.loss = criterion_nored
            self.cfg.dss_args.eta = self.cfg.optimizer.lr
